...
def create_new_node_network_sqlite(num_input_nodes=2048):
    ...
    existing_conn_ids = {}
    insert_query = "INSERT INTO connections"
    insert_query += " (id, inputting_node, outputting_node, weight)"
    insert_query += " VALUES"
    loop_time = time.time()
    for i in range(num_nodes):
        for _ in range(num_connections_per_node):
            ...
            insert_query += ''' ("{0}", {1}, {2}, {3}),'''.format(conn_id, dest, i, random_weight)
    insert_query = insert_query[:-1]
    print("Loop took:", loop_time - time.time())
    conn.execute(insert_query)
    conn.commit()
    print("Loop and commit took:", loop_time - time.time())
